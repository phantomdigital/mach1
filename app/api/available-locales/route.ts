  ...
  try {
    ...
    try {
      if (docType === 'home') {
        ...
        document = await client.getByUID(docType as any, uid, { lang });
      } else {
        return NextResponse.json(
    ...
    if (document.alternate_languages && document.alternate_languages.length > 0) {
      ...
      document.alternate_languages.forEach((alt: any) => {
    ...
    for (const locale of allLocales) {
      ...
      try {
        if (docType === 'home') {
          ...
          await client.getByUID(docType as any, uid, { lang: locale });
        }
