  ...
  try {
    ...
    return (
      <>
        ...
        {/* Main Footer Content */}
        <footer className="bg-dark-blue text-neutral-200 relative overflow-hidden">
          <div className="w-full max-w-[88rem] mx-auto px-4 lg:px-8 py-16 lg:py-24">
                      ...
                      {section.links.map((link, linkIndex: number) => (
                        link.label && (
                          <li key={linkIndex}>
                            <PrismicNextLink
                              field={link.link}
                              className="text-neutral-300 text-xs lg:text-sm font-normal hover:text-white hover:underline transition-colors"
                            >
                              {link.label}
                            </PrismicNextLink>
                          </li>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="space-y-3 lg:space-y-4">
                            {footer.data.contact_section[0].addresses.map((item, index: number) => (
                              item.address && (
                                <p key={index} className="text-neutral-200 text-xs lg:text-sm font-normal leading-relaxed">
                                  {item.address}
                                </p>
                              )
                            ))}
                            ...
                            {footer.data.contact_section[0].phone_contacts.map((contact, index: number) => (
                              <div key={index}>
                                {contact.contact_name && (
                                  <h5 className="text-neutral-400 text-xs font-medium uppercase tracking-wider mb-2 lg:mb-3">
                                    {contact.contact_name}
                                  </h5>
                                )}
                                {contact.phone_number && (
                                  <p className="text-neutral-200 text-xs lg:text-sm font-normal">
                                    {contact.phone_number}
                                  </p>
                                )}
                              </div>
                    ...
                    <div className="flex items-center gap-6 lg:gap-8 xl:gap-12 mt-auto">
                      {footer.data.social_media.map((social, index: number) => (
                        <PrismicNextLink
                          ...
                          aria-label={`Visit us on ${social.platform || 'social media'}`}
                        >
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center gap-4 lg:gap-6">
              ...
              <div className="flex items-center flex-wrap justify-center lg:justify-start gap-3 lg:gap-4 order-2 lg:order-1">
                {footer.data.legal_links && footer.data.legal_links.slice(0, 2).map((legalLink, index: number) => (
                  <PrismicNextLink
                    ...
                    className="text-neutral-400 text-xs font-normal hover:text-white hover:underline transition-colors"
                  >
                    {legalLink.label}
                  </PrismicNextLink>
                ))}
              ...
              {/* Copyright - Right */}
              <p className="text-neutral-400 text-xs font-normal text-center lg:text-right order-1 lg:order-2">
