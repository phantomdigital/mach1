...
const HeroBlockAnimation = React.lazy(() => import("./hero-block-animation"));
...
const HeroBlock = ({ slice }: HeroBlockProps): React.ReactElement => {
  ...
  // Content that renders immediately (no animation dependency)
  const heroContent = (
  ...
  return (
    ...
    >
      <Suspense fallback={<div>{heroContent}</div>}>
        <HeroBlockAnimation>
          {heroContent}
        </HeroBlockAnimation>
      </Suspense>
